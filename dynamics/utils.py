...
def load_data(path, train=True):
    ...
    if os.path.isfile(path):
        print(f'Trajectory Found. Loading {data_type} data...')
        buffer = np.load(path)['arr_0']
    else:
        print('Trajectories not found. Please use dynamics/data/generate.py to generate train and test trajectories...')
        exit(1)
    return buffer
